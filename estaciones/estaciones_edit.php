<?php 
    ...
    $type = $_GET['type'];
    $pag = $pag."?type=".$type."&id_estaciones=".$_GET['id_estaciones'];

    if (isset($_GET['id_estaciones'])) {
    	$id_estaciones = $_GET['id_estaciones'];

    	$gett = "SELECT * FROM estaciones WHERE id_estaciones=$id_estaciones";
    	$query = mysqli_query($conexion, $gett);
    	$row = mysqli_fetch_assoc($query);

    	$row_nombre_estaciones = $row["nombre"];
    	$row_cupos_estaciones = $row["cupos"];
    ...
    if (isset($_POST["enviar"])) {
		...
		$sql = "UPDATE estaciones SET nombre='$nombre', cupos='$cupos', cupos_vol='$cantidad_x_voluntario' WHERE id_estaciones=$id_estaciones";
									...
									<div class="row">
										<div class="col-md-3">
											<div class="form-group">
												...
												<input tabindex="1" autocomplete="off" type="text" class="form-control" id="nombre" name="nombre" placeholder="Ingrese nombre de la estación" value="<?php echo $row_nombre_estaciones ?>">
											</div>
										</div>
										<div class="col-md-3">
											<div class="form-group">
												<label for="cupos">Cupos disponibles</label>
												<input tabindex="2" autocomplete="off" type="text" class="form-control" id="cupos" name="cupos" placeholder="Ingrese cantidad de cupos" value="<?php echo $row_cupos_estaciones ?>">
											</div>
										</div>
									</div>
